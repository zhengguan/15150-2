(* thanks to Michael Sullivan for this code *)
...
signature ANSI =
  sig
    ...
    val colorStr : string -> { fg : full_color option , bg : full_color option } -> string
  end
...
struct
  ...
  fun fullColor {fg, bg} =
    (case fg of
      SOME (fgcolor, is_bold) => bold is_bold ^ fgColor fgcolor
    | NONE => "") ^
    (case bg of
      SOME (bgcolor, is_bold) => bold is_bold ^ bgColor bgcolor
    | NONE => "")
  fun colorStr s colors = fullColor colors ^ s ^ reset
